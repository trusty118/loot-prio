...
- **`data/bis.json`** - which items are best-in-slot for which spec, keyed by the same
  identifiers, and marked `phase`, `multiPhase` or `expansion`. 329 entries across all 27
  specs, generated by `python3 verify/fetch_bis.py` from the guides listed in
  `verify/bis-sources.json`; hand edits survive a re-run.
