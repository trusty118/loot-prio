...
**182 of the 195 rows are theirs.** The other 13 are T6 items the videos never mention, added
so the loot tables are complete; they hold no priority, carry a **"not in the guide"** tag in
the table, and are listed in `verify/missing-items.md`.
...
**The best-in-slot marks are a separate layer and are not zatar's** — the videos gave loot
priorities, not per-spec BiS lists. They come from [Wowhead](https://www.wowhead.com/tbc)'s
per-spec Phase 3 BiS guides (every source URL is recorded in `verify/bis-sources.json`), with
how long an item stays BiS derived from wowsims/tbc's P4 and P5 gear presets.
...
195 records: Black Temple 117, Mount Hyjal 66, Crafted 12.

182 of those are zatar's. The other 13 are items he never covered, found by auditing
Wowhead's Phase 3 BiS guides against this dataset (`verify/missing-items.md`); they carry
`"unsourced": true`, an empty `priority`, and a **"not in the guide"** tag in the table.
...
- **`data/bis.json`** - which items are best-in-slot for which spec, keyed by the same
  identifiers, and marked `phase`, `multiPhase` or `expansion` for how long it stays BiS.
  366 entries across all 28 specs, generated by `python3 verify/fetch_bis.py` from the
  guides listed in `verify/bis-sources.json`; hand edits survive a re-run. Feral druid is
  split into bear and cat, which gear too differently to share one set.
